<? onlinecheck($pdo); ?><h2>Header deiner Firma ändern</h2>

<form action="#" method="post" enctype="multipart/form-data">
    Wähle deinenen neuen Firmen-Header:
...
if(isset($_POST['submit'])) {
  ...
  $statement9->execute(array($_SESSION['username']));
    while($row3 = $statement9->fetch()) {
      $datei = "img/firmenheader/".$row3['firmen_header'];
        unlink ($datei);
      }


$target_dir = "img/firmenheader/";
...
$stmt1 = $pdo->prepare("UPDATE firmen SET firmen_header = :logo WHERE email = :email");
