<? onlinecheck($pdo); ?><?=pfeil_back();?><h2>Auszeichnungen</h2>
<?
$user_id = filter_input(INPUT_GET, "terebskptlq", FILTER_SANITIZE_STRING);

$sql_verw = $pdo->prepare("SELECT * FROM auszeichnungen WHERE einmalige_id = :uniq_id ORDER BY datum DESC");
...
if($i == 1) {
  echo "<td width='850px' align='left' valign='top' style='border-bottom:1px solid grey;border-top:1px solid grey;'>".$row_v['auszeichnung']."</td><tr>";
} else {
  echo "<td width='850px' align='left' valign='top' style='border-bottom:1px solid grey;'>".$row_v['verstoss']."</td><tr>";
}
